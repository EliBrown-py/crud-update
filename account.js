...
const updateHandler = (index) => {
  ...
  // Get the existing data for the selected animal
  const selectedAnimal = animals[index];

  // Create an update element showing the text fields with the existing values
  let updatePara = $(`
    <div class="updatePara">
      <input type="text" id="update_name" value="${selectedAnimal.animal_name}" size="6"/> 
      is a 
      <input type="text" id="update_breed" value="${selectedAnimal.breed}" size="8" />, 
      and is 
      <input type="text" id="update_age" value="${selectedAnimal.age}" size="1" /> 
      years old. Feed her at 
      <input type="text" id="update_feed_times" value="${selectedAnimal.feed_times}" size="16">
      with 
      <input type="text" id="update_scoops" value="${selectedAnimal.scoops}" size="1">
      scoops of 
      <input type="text" id="update_food" value="${selectedAnimal.food}" size="11">. 
      <input type="text" id="update_warning" value="${selectedAnimal.warning}" size="30">.
      <button class="save_update update-button">Save</button>
    </div>`);
  // Replace the existing content with the selected content
  $(`.animals .${panel_class} .displayLine`).eq(index).html(updatePara);

  // Add a click handler for the save update button
  $(".save_update").on("click", function () {
    let updatedAnimal = {
      breed: $("input#update_breed").val(),
      animal_name: $("input#update_name").val(),
      age: $("input#update_age").val(),
    ...
    //Update the animals array with the new data
    ...
    // Replace the update paragraph with the updated animal line
    $(`.animals .${panel_class} .displayLine`).eq(index).html(updatedAnimalLine);

    // Add click handlers for the buttons  in the updated display line
    $(".displayLine button").on("click", (e) => {
      const target = $(e.target);
      console.log(target);
      console.log(target["0"]);
